%% Code Workshop 4 val -- TSO Célia et MERBOUCHE Mouloud %%
...
ddqfval = ddqfval(1:J, :);
taufval = taufval(1:J, :);
q_measval = q_meas_validation(1:J, :);
Tauval = tau_meas_validation(1:J, :);

%p.15
nn = (length(t)-1)/380;
for k = 1 : nn
t_subval(1,k) = t(1,(k-1)*380+1);
q_subval(:,k) = qfval(:,(k-1)*380+1);
dq_subval(:,k) = dqfval(:,(k-1)*380+1);
ddq_subval(:,k) = ddqfval(:,(k-1)*380+1);
tau_subval(:,k) = taufval(:,(k-1)*380+1);
end

tau_idval = sym(zeros(n,nn));
...
tau_idval(:,mm) = simplify(vpa(M * qpp + N * qp + G - Fv .* qp - Fc .* sign(qp)));
end

save tau_idval tau_idval
save tau_subval.mat tau_subval
